<template>
  <div class="box-card">
    <!-- {{ form.area }}
    {{ form.geojson }}-->
    <van-form @submit="() => { onSubmit(); onCancel() }">
      <van-cell-group inset>
        ...
        <van-field v-model="form.pictrues" name="pictrues" placeholder="地块图片（可修改）" label="地块图片" />
      </van-cell-group>
...
function onCancel() {
  if (!map || !drawS) return;
  map.removeInteraction(drawLInteraction);
  map.removeInteraction(modifyLInteraction);
  drawS.clear();
  map.removeLayer(drawL);
  emit('close')
}
...
function startDrawLayer() {
  map.addInteraction(drawLInteraction);
  map.addInteraction(modifyLInteraction);
}
